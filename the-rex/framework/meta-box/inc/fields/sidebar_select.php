...
if ( ! class_exists( 'RWMB_Sidebar_Select_Field' ) )
{
	class RWMB_Sidebar_Select_Field extends RWMB_Field
	{
		...
		{
			wp_enqueue_style( 'rwmb-sidebar-select', RWMB_CSS_URL . 'select.css', array(), RWMB_VER );
		}
		...
		{
			$html = sprintf(
				'<select class="rwmb-sidebar-select" name="%s" id="%s" size="%s"%s>',
				$field['field_name'],
				$field['id'],
				$field['size'],
		...
		{
			$html = '';
            $html .=  '<option value="global"'.selected( in_array( 'global', (array) $meta ), true, false ).'>'.esc_html__( "Global Setting", "the-rex").'</option>';
            $html .=  '<option value="disable"'.selected( in_array( 'disable', (array) $meta ), true, false ).'>'.esc_html__( "Disable Sidebar", "the-rex").'</option>';
