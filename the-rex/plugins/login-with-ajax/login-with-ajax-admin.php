...
class LoginWithAjaxAdmin{
	...
	function admin_notices() {
		...
		<div id='lwa-warning' class='updated fade'>
			<p>
				...
				<a href="<?php echo esc_url( site_url() ); ?>/wp-admin/options-general.php?page=login-with-ajax">Settings</a> | 
	...
	function options() {
		...
		if( !empty($_POST['lwasubmitted']) && current_user_can('list_users') && wp_verify_nonce($_POST['_nonce'], 'login-with-ajax-admin'.get_current_user_id()) ){
			...
			?>
			<div class="updated"><p><strong><?php esc_html_e('Changes saved.', 'the-rex'); ?></strong></p></div>
			<?php
		}else{
...
The team at %BLOGNAME%', 'login-with-ajax');
							...
							<div>
								...
								<p class="submit">
									<input type="submit" class="button-primary" value="<?php esc_html_e('Save Changes', 'the-rex') ?>" />
								</p>
							</div>		
						</form>
					</div>
